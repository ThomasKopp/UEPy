...
class OllamaTranslatorApp:
    ...
    def __init__(self, root):
        ...
        if self.use_threads:
            # First: just check status; then try to start if offline.
    ...
    def _check_server_status(self):
        ...
        try:
            requests.get(self.get_api_base_url(), timeout=1)
            self._set_server_status("Server: online", ok=True)
            return True
        except requests.exceptions.RequestException:
            self._set_server_status("Server: offline", ok=False)
    ...
    def upload_pdf(self):
        ...
        try:
            ...
            if not content:
                messagebox.showwarning("Leere PDF", "Die PDF enthält keinen extrahierbaren Text.")
                return
            self.input_text.delete('1.0', tk.END)
            self.input_text.insert('1.0', content)
            self.update_translate_button_state()
            ...
            messagebox.showerror("Datei nicht gefunden", f"Die Datei '{filepath}' konnte nicht geöffnet werden.")
            self.show_error(f"Datei nicht gefunden: {filepath}")
        except Exception as e:
            messagebox.showerror("PDF-Lesefehler", f"PDF konnte nicht gelesen werden: {e}")
            self.show_error(f"PDF-Lesefehler: {filepath}")
    ...
    def _extract_pdf_text(self, filepath):
        ...
        if PaddleOCR and convert_from_path:
            try:
                lang = "german" if self.direction_var.get() == "de-en" else "en"
                ocr = PaddleOCR(lang=lang, show_log=False, use_angle_cls=True)
